...
namespace Shiro {
TextGraphic::TextGraphic(const Screen &screen, const FontAsset &font, const std::string &text, const int x, const int y, const float scale,
    ...
    for(std::string line; std::getline(lines, line);)
    {
        if(line.size() > 0u)
        {
            ...
            for(std::size_t i = 0u; i < line.size(); i++)
            {
                ...
                textData.push_back({font.pages[ch.page],
                                    make_frect(int(ch.x), int(ch.y), int(ch.width), int(ch.height)),
                                    make_frect(int(scale * (printX + ch.xoffset)), int(scale * (printY + ch.yoffset)), int(scale * ch.width), int(scale * ch.height))});
