...
bool CoreState::is_left_input_repeat(unsigned delay) { return keys.left && hold_dir == Shiro::DASDirection::LEFT && hold_time >= delay; }

bool CoreState::is_right_input_repeat(unsigned delay) { return keys.right && hold_dir == Shiro::DASDirection::RIGHT && hold_time >= delay; }

bool CoreState::is_up_input_repeat(unsigned delay) { return keys.up && hold_dir == Shiro::DASDirection::UP && hold_time >= delay; }

bool CoreState::is_down_input_repeat(unsigned delay) { return keys.down && hold_dir == Shiro::DASDirection::DOWN && hold_time >= delay; }

CoreState::CoreState(Shiro::Settings &settings)
    : screen(Shiro::Version::DESCRIPTOR, static_cast<unsigned>(settings.videoScale * 640.0f), static_cast<unsigned>(settings.videoScale * 480.0f), 640, 480,
             float(settings.videoScale))
    , settings(settings)
    , bg(screen)
{
...
bool CoreState::init()
{
    ...
    try
    {
        ...
        });
        {
            ...
            if(readSuccess)
            {
                std::string musicNames[] = {"multi_editor_bgm",
                                            "tracks0",
                                            "tracks1",
                                            "tracks2",
                                            "tracks3",
        ...
        const int numJoysticks = get_num_joysticks();
        if(numJoysticks > 0)
        {
            for(int i = 0; i < numJoysticks; i++)
            {
                SDL_Joystick *joystick;
                if((joystick = SDL_OpenJoystick(i)))
                {
                    std::cerr << "Attached joystick \"" << SDL_GetJoystickName(joystick) << "\" at index " << i << std::endl;
                    SDL_CloseJoystick(joystick);
                }
                else
                {
            ...
            if(settings.controllerBindings.name != "")
            {
                ...
                for(int i = 0; i < numJoysticks; i++)
                {
                    if((joystick = SDL_OpenJoystick(i)))
                    {
                        if(SDL_GetJoystickName(joystick) == settings.controllerBindings.name)
                        {
                            settings.controllerBindings.controllerIndex = i;
                            settings.controllerBindings.controllerID = SDL_GetJoystickID(joystick);
                            break;
                        }
                        else
                        {
            ...
            if(!joystick && settings.controllerBindings.controllerIndex >= 0 && settings.controllerBindings.controllerIndex < get_num_joysticks())
            {
                if((joystick = SDL_OpenJoystick(settings.controllerBindings.controllerIndex)))
                {
                    settings.controllerBindings.controllerID = SDL_GetJoystickID(joystick);
                }
            }
            ...
            {
                std::cerr << "Joysticks are enabled" << std::endl
                          << "Name: " << SDL_GetJoystickNameForID(settings.controllerBindings.controllerIndex) << std::endl
                          << "Index: " << settings.controllerBindings.controllerIndex << std::endl
                          << "Buttons: " << SDL_GetNumJoystickButtons(joystick) << std::endl
                          << "Axes: " << SDL_GetNumJoystickAxes(joystick) << std::endl
                          << "Hats: " << SDL_GetNumJoystickHats(joystick) << std::endl;
            }
            else
            {
                joystick = nullptr;
        ...
        // check(Shiro::RenderCopy(cs->screen, blank, NULL, NULL) > -1, "SDL_RenderTexture: Error: %s\n", SDL_GetError());
...
void CoreState::run()
{
    ...
    bool slept = true;
    for(nanotime_step_init(&stepper, NANOTIME_NSEC_PER_SEC / fps, nanotime_now, nanotime_sleep); running;
            ...
            : (realFrameTime = (currentTime = nanotime_now()) - lastTime, lastTime = currentTime))
    {
        ...
        if(realFrameTime > 0u)
        {
            std::cerr << "real FPS: " << std::setprecision(10) << NANOTIME_NSEC_PER_SEC / static_cast<double>(realFrameTime) << std::endl;
        }
        else
        {
            std::cerr << "real FPS: ???" << std::endl;
        }
        std::cerr << "game FPS: " << std::setprecision(10) << (settings.vsync && settings.vsyncTimestep && videoFPS > 0.0 ? videoFPS : fps) << std::endl;
#endif
        ...
        */
        ...
        {
          public:
            OldGfxEntity(const std::size_t layerNum, const std::function<void()> drawLambda) : layerNum(layerNum), drawLambda(drawLambda) {}
        ...
        gfx.push(std::make_unique<OldGfxEntity>(Shiro::GfxLayer::base, [this] {
            if(p1game)
            {
                p1game->draw(p1game);
            }
        ...
        gfx.push(std::make_unique<OldGfxEntity>(Shiro::GfxLayer::buttons, [this] { gfx_drawbuttons(this, 0); }));

        // TODO: Create entities in the code for emergency buttons, then remove this.
        gfx.push(std::make_unique<OldGfxEntity>(Shiro::GfxLayer::emergencyButtons, [this] { gfx_drawbuttons(this, EMERGENCY_OVERRIDE); }));
...
#define FONT(name, sheetName, outlineSheetName, charW, charH) load_bitfont(&assets->name, &assets->sheetName, &assets->outlineSheetName, charW, charH);
#include "fonts.h"
#undef FONT
}
...
#undef FONT
    ...
    if(cs->joystick && SDL_JoystickConnected(cs->joystick))
    {
        SDL_CloseJoystick(cs->joystick);
    }
...
// TODO: Make this only read in events and update state; move out all logic interpreting the new state into the game/menu code, such as DAS processing. And
// rename this to pollEvents once that's all done.
...
#undef CHECK_KEYDOWN
                ...
                {
                    case SDLK_V:
                        if((keyMod & SDL_KMOD_CTRL) && text_editing && text_insert)
                        {
                    ...
                    case SDLK_X:
                        if((keyMod & SDL_KMOD_CTRL) && text_editing && text_cut)
                        {
                            text_cut(this);
                        }
                        break;
...
#undef CHECK_KEYUP
            ...
            case SDL_EVENT_TEXT_INPUT:
                if(text_editing)
                {
                    // Only insert text if not a Ctrl+C/V/X/A event
                    if(!((event.text.text[0] == 'c' || event.text.text[0] == 'C') && (SDL_GetModState() & SDL_KMOD_CTRL)) &&
                       !((event.text.text[0] == 'v' || event.text.text[0] == 'V') && (SDL_GetModState() & SDL_KMOD_CTRL)) &&
                       !((event.text.text[0] == 'x' || event.text.text[0] == 'X') && (SDL_GetModState() & SDL_KMOD_CTRL)) &&
                       !((event.text.text[0] == 'a' || event.text.text[0] == 'A') && (SDL_GetModState() & SDL_KMOD_CTRL)))
                    {
                        if(text_insert)
                        {
                            std::string inputText(event.text.text);
                            text_insert(this, inputText.data());
                        }
                    }
                }
    ...
    SDL_SetRenderScale(screen.renderer, screen.render_scale, screen.render_scale);
    ...
    if(screen.render_scale != prevRenderScale)
    {
        ...
        if(screen.target_tex != nullptr)
        {
            ...
            screen.target_tex = SDL_CreateTexture(screen.renderer,
                                                  SDL_GetWindowPixelFormat(screen.window),
                                                  SDL_TEXTUREACCESS_TARGET,
                                                  static_cast<int>(640.0 * screen.render_scale),
                                                  static_cast<int>(480.0 * screen.render_scale));
            if(screen.target_tex == nullptr)
            {
...
#undef CHECK_BINDINGS
    ...
    {
        if(mouse.shown && SDL_GetTicks() >= mouse.hideTicks)
        {
            SDL_HideCursor();
            mouse.shown = false;
        }
...
bool CoreState::button_emergency_inactive() { return !button_emergency_override; }
...
void CoreState::gfx_buttons_input()
{
    ...
    for(auto it = gfx_buttons.begin(); it != gfx_buttons.end(); it++)
    {
        ...
        if(mouse.logicalX < b.x + b.w && mouse.logicalX >= b.x && mouse.logicalY < b.y + b.h && mouse.logicalY >= b.y)
            b.highlighted = 1;
        else
            b.highlighted = 0;
